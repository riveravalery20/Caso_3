...
# Definir nombres cortos para usar en los círculos de correlación
...
```{r preparacion-datos}
# Extraer datos DIRECTAMENTE del objeto res.pca (prcomp)
# Las coordenadas están en res.pca$rotation (loadings)
...
# Calcular contribuciones CORRECTAMENTE como lo hace factoextra
# La contribución es relativa al total de inercia en las dimensiones mostradas
# Para PC1 y PC2: contrib = (coord^2 * eigenvalue / suma_eigenvalues_PC1_PC2) * 100
# Para PC1 y PC3: contrib = (coord^2 * eigenvalue / suma_eigenvalues_PC1_PC3) * 100
...
# Generar gráfico de silueta base
g_sil <- fviz_nbclust(
  coordenadas_acp, 
  FUN = hcut,
  method = "silhouette",
  ...
  hc_metric = "euclidean",
  k.max = 10,
  barfill = "#3498DB",     # Azul profesional
  barcolor = "#2980B9",    # Azul más oscuro para bordes
  linecolor = "#E74C3C",   # Rojo para la línea
  ggtheme = theme_minimal()
) + 
  labs(
...
# Calcular límites del gráfico para mantener todo dentro del cuadro
y_max <- max(g_sil$data$y) * 1.15  # 15% más de espacio para la anotación
x_limits <- c(0.5, 10.5)  # Límites ajustados para eje X

# Construir gráfico final
g_sil_final <- g_sil +
  # Resaltar el punto óptimo
  geom_point(data = g_sil$data[which.max(g_sil$data$y),], 
             aes(x = clusters, y = y), 
             color = "#E74C3C", size = 5, shape = 21, 
             stroke = 2, fill = "white") +
  ...
  # Anotación del valor óptimo - POSICIÓN AJUSTADA
  annotate("text",
           x = optimo_clusters,
           y = y_max * 0.95,  # Posición dentro de los límites
           label = paste("CLÚSTERES ÓPTIMOS: k =", optimo_clusters),
           color = "#2C3E50",
           fontface = "bold",
           size = 5,
           hjust = 0.5) +
  # Ajustar escalas para mantener todo dentro del cuadro
  scale_x_continuous(breaks = 1:10, limits = x_limits) +
