...
# Calcular cos2 para cada país
cos2_values <- res.pca$ind$cos2[, 1] + res.pca$ind$cos2[, 2]

# Seleccionar top 20 países con mejor representación
top_countries <- names(sort(cos2_values, decreasing = TRUE))[1:20]

# Gráfico solo con países destacados
p3  <- fviz_pca_ind(res.pca,
             col.ind = "cos2",
             select.ind = list(cos2 = 20),  # Solo top 20
             ...
             alpha.ind = 0.8,
             ggtheme = theme_minimal()) +
  labs(title = "PAÍSES CON MEJOR REPRESENTACIÓN EN EL PLANO FACTORIAL",
       subtitle = "Top 20 países por calidad de representación (cos2)",
       caption = "Los países con cos2 bajo tienen representación menos fiable") +
  theme(
...
nombres_cortos <- c(
  "Exp_vida_mujer" = "Expectativa_de_vida_mujer",
  "Exp_vida_hombre" = "Expectativa_de_vida_hombres",
  "Acc_transito" = "Accidentes_de_transito",
  "Enf_card_mujer" = "Enfermedades_cardiacas_cancer_diabetes_mujeres",
  "Enf_card_hombre" = "Enfermedades_cardiacas_cancer_diabetes_hombres",
  "Polucion_mujer" = "Polucion_del_aire_mujeres",
  "Polucion_hombre" = "Polucion_del_aire_hombres",
  "Envenena_mujer" = "Envenenamiento_accidental_mujeres",
  "Envenena_hombre" = "Envenenamiento_accidental_hombres",
  "Mort_adulta_mujer" = "Tasa_de_mortalidad_mujeres_adultas",
  "Mort_adulta_hombre" = "Tasa_de_mortalidad_hombres_adultos",
  "Mort_inf_mujer" = "Tasa_de_mortalidad_infantil_mujeres",
  "Mort_inf_hombre" = "Tasa_de_mortalidad_infantil_varones",
  "Mort_neonatal" = "Tasa_de_mortalidad_neonatal",
  "Mort_temp_mujer" = "Tasa_de_mortalidad_infantil_temprana_mujeres",
  "Mort_temp_hombre" = "Tasa_de_mortalidad_infantil_temprana_hombres",
  "Muertes_infantiles" = "Numero_de_muertes_infantiles",
  "Muertes_neonatales" = "Numero_de_muertes_neonatales",
  "Suicidios_mujer" = "Tasa_de_suicidios_mujeres",
  "Suicidios_hombre" = "Tasa_de_suicidios_hombres",
  "Sup_65_mujer" = "Supervivencia_hasta_los_65_anos_mujeres",
  "Sup_65_hombre" = "Supervivencia_hasta_los_65_anos_hombres"
)

# Aplicar nombres cortos a los datos
Muertes_short <- Muertes
colnames(Muertes_short)[-1] <- names(nombres_cortos)[match(colnames(Muertes)[-1], nombres_cortos)]

# Rehacer el ACP con nombres cortos
Muertes_df_short <- Muertes_short %>% 
  select(-Pais) %>%
  mutate(across(everything(), as.numeric)) %>%
  scale()
rownames(Muertes_df_short) <- Muertes_short$Pais

res.pca_short <- prcomp(Muertes_df_short, scale = FALSE)

p4 <- fviz_pca_var(res.pca_short,
             axes = c(1, 3),
             select.var = list(contrib = 10),  # SOLO 10 variables más importantes
             col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE,
             alpha.var = 0.9,
             shape.var = 17,
             labelsize = 5,   # Texto más grande para mejor legibilidad
             pointsize = 2.5,
             ggtheme = theme_minimal()) +
  labs(title = "TOP 10 VARIABLES CON MAYOR CONTRIBUCIÓN",
       subtitle = "Componentes Principales 1 vs 3 - Variables Más Influyentes",
       caption = "Las 10 variables que más contribuyen a la variabilidad de los datos") +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
